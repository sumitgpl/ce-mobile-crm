...
function SugarCrmDeleteExistingMeeting() {
    if (confirm(RES_ACTION_CONFIRM_DELETE)) {
        $.get(CurrentServerAddress + '/service/v2/rest.php', {
            ...
            '"module":"Meetings",' +
            '"name_value_list":[{"name":"id","value":"' + CurrentMeetingId + '"},' +
            ...
            if (data !== undefined) {
                ...
                $('#AllMeetingsListDiv').children().remove('li');
                $.mobile.hidePageLoadingMsg();
                $.mobile.changePage('#MeetingsListPage');
            }
        });
    }
}
